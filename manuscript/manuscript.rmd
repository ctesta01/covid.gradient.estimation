...
```{r two-variables-over-time, fig.cap="The changing interacted effect of the ICE for racialized economic segregation and age over time. A) May 2020, B) November 2020, C) May 2021, D) October 2021, E) February 2022, F) August 2022. An animated version is available online at [https://github.com/ctesta01/covid.gradient.estimation/blob/main/analysis/05_two_variables_at_a_time/animation_ICEraceinc_age/readme.md](https://github.com/ctesta01/covid.gradient.estimation/blob/main/analysis/05_two_variables_at_a_time/animation_ICEraceinc_age/readme.md)"}
...
formula specified as `deaths ~ te(latitude, longitude, time, d=c(2,1))` where
`d=c(2,1)` indicates that `latitude` and `longitude` share the same dimensions
(i.e., both are spatial and in units of degrees) while the `time` data are in separate
units. 
...
rural and low population density, the spatiotemporally smoothed GAM estimates
...
```{r spatiotemporal-panel-figure, fig.cap = "Spatiotemporally smoothed COVID-19 mortality estimates from a GAM fit to data from March 2020-August 2022. Panels show months selected to highlight the changing spatial patterns of COVID-19 mortality risk over time. An animated version is available online at []()"}
